...
const TEMPLATE = `<div class="dp-container">
  <div class="dp-header">
    ...
    <div class="dp-item dp-right vux-datetime-confirm" style="color:#0e90e0" data-role="confirm">ok</div>
  </div>
  ...
  show (value) {
    ...
    if (self.container) {
      ...
      each(TYPE_MAP, function (type) {
        ...
        let data
        if (type === 'day') {
          var _monthRow = self.monthScroller.options.data
          var _defaultValue = self.monthScroller.options.defaultValue
          var _firstSelectMonth = ''
          // 检查当月 是否 在 monthRow 列表里面
          // 若不在则 取 monthRow 里的第一个 月
          _monthRow.forEach( (val, key)=>{
            if( val.name.indexOf(_defaultValue ) > -1 || val.value.indexOf(_defaultValue ) > -1 ){
              _firstSelectMonth = newValueMap.month
            }
          })

          console.log( newValueMap , '《《《datetimePicker.js', '219行'  )

          if( !_firstSelectMonth ){
            _firstSelectMonth = _monthRow[0].value
          }
          console.log( _firstSelectMonth , ' 读取第一次的日期月份 《《《datetimePicker.js', '220行'  )

          data = self._makeData(type, trimZero(newValueMap.year), trimZero( _firstSelectMonth ))
        } else if (type === 'hour') {
