...
<template>
    <div>
      <div class="v-swipeout">
        <div class="v-swipeout-item"
             v-Clickoutside:touchstart="_setClose"
             @touchstart="start"
             @mousedown="start"
             @touchmove="move"
             @mousemove="move"
             ...
             @mouseup="end"
             @touchcancel="end">
          <div class="v-swipeout-button-box v-swipeout-button-box-left" :style="leftButtonBoxStyle" v-show="distX >= 0">
            <slot name="left-menu"></slot>
          </div>
          <div class="v-swipeout-button-box" :style="rightButtonBoxStyle" v-show="distX <= 0">
            <slot name="right-menu"></slot>
          </div>
          <div class="v-swipeout-content" :style="[styles, {backgroundColor: contentBg }]" @mousedown="onContentClick" @touchstart="onContentClick" ref="content">
    ...
    methods: {
      ...
      setOffset (x, animated = false, force) {
        ...
        if (animated && this.target) {
          this.target && this.target.classList.add('v-swipeout-content-animated')
          var cb = (function (self, target) {
            return function () {
              target.classList.remove('v-swipeout-content-animated')
